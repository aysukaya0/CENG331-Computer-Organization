...
# In fetch stage, the address that should be fetched is determined by W_valM, so I added "W_icode == IJTAB : W_valM;" to the f_pc list. Also,
# since jtab is a valid instruction, I added IJTAB to instr_valid list. jtab uses register B (%rB) and a constant C
# value to update, so it needs registers and valC. Therefore, I put IJTAB to need_regids and need_valC list. As jtab uses register B, 
# D_rB should be used as d_srcB and any other register do not need to be used in decode stage. In execute stage, it uses valC and
# valB, so I added IJTAB to E_valC list in aluA and to E_valB list in aluB. In memory stage, jtab only reads from memory which is the
# value stored in C(%rB) but it does not write anything to memory. Thus, I added IJTAB to mem_read list an it uses M_valE as memory
# address, so I also added IJTAB to M_valE list in mem_addr.

# jtab resembles to ret instruction with respect to pipeline register control. We should stall Pipeline Register F at fetch while jtab passes
# through pipeline which means IJTAB in { D_icode, E_icode, M_icode }.So I added this to F_stall condition using or operator. After that, we
# we should inject a bubble to Pipeline Register D. Thus I added IJTAB in { D_icode, E_icode, M_icode } && !(E_icode in { IMRMOVQ, IPOPQ } 
# && E_dstM in { d_srcA, d_srcB }) to D_bubble condition using or operator. We should not stall or inject a bubble into Pipeline Register E, 
# W and M. They stay normal.
